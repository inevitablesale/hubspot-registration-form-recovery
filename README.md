...
- **Batch CRM updates** – Looks up contact IDs and writes consent changes in batches of up to 100 contacts at a time so very
  large recovery runs (10k+ submissions) finish quickly and stay within HubSpot rate limits.
...
3. **Find matching contact** – Uses the HubSpot CRM batch read endpoint (`/crm/v3/objects/contacts/batch/read`) to resolve up to
   100 emails at a time to their HubSpot contact IDs.
4. **Update contact** – Sends the consent changes with the batch update endpoint (`/crm/v3/objects/contacts/batch/update`) so
   the service makes at most one CRM request per 100 contacts. If the contact is missing or an error occurs, the service records
   the skipped/error count but continues processing the remaining submissions.
